...
void INIParser::Parser::AddFile(const std::filesystem::path &path) {
    std::fstream fileIn(path, std::ios::binary | std::ios::in);
    if(!fileIn.is_open())
        throw Exception("Can't open file \'config.ini\'");
...
void INIParser::Parser::AddString(const std::string_view &str) {
    Reset();
    for (char c : str)
        ProcessChar(c);
}

void INIParser::Parser::Reset() {
    m_currentSection = "";
    m_currentKey = "";
    m_currentValue = "";
...
void INIParser::Parser::ProcessChar(char c) {
    // Catch controle keys
    if (std::iscntrl(c))
        switch (c) {
            case '\t':
...
void INIParser::SAXParser::ParseKVPair(const std::string &section, const std::string &key, const std::string &value) {
    if (m_callback)
        m_callback(section, key, value);
}

void INIParser::DOMparser::ParseKVPair(const std::string &section, const std::string &key, const std::string &value) {
